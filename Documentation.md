...
![Earliest commit prototype](./Documentation_images/Earliest_commit_prototype.png)

_The board partway through implementing the feedback_
...
24. There should be a command to enable cheats
25. There should be a command to alter a player's money, both for testing and to allow players to customise their game experience; this should be a cheat
26. There should be the ability to view information about players on demand
27. There should be the ability to view the card for an animal on demand
28. There should be the ability for players to trade animals and/or money on demand; this should be a cheat if one of the players is an AI, as the AI cannot agree to a trade
...
33. One AI - the 'hard' AI - should only buy/upgrade an animal if it has determined it is a good investment and that doing so would not put it at risk of bankruptcy before its next turn
34. One AI - the 'expert' AI - should predict the possible future states of the game and [maximise the worst-case probability of it winning](https://en.wikipedia.org/wiki/Minimax)

#### Saving:

35. There should be the ability to save and load
36. Saving should write every important piece of data to disk, such that the program can be completely relaunched and the save file can be loaded to resume the game with no noticable differences
37. There should be data cleaning to prevent invalid names - e.g. ones containing slashes - from being used

#### Ending:

38. When players go 'into debt' (have negative money), they should be informed and have one turn to get out of debt or else be eliminated
39. When players are eliminated, their animals should have their owner cleared, but should not return to their base level, thus making them more valuable
40. When there is only one player left in, the game should end and they should be declared the winner. If all remaining players are eliminated on the same turn, the winner should be decided by which player had the smallest 'debts' (had the least negative money)
41. After the game ends, a 'game review' graph should be generated to let players see how much many they had throughout the game

#### Graph:

42. The graph should show how much money each player had on each turn
